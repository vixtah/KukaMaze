			 ...
			 rll_planning_project::Move::Response &resp)
{
	...
	if ((move_dist < 0.005 && move_dist > 0.0005) || (move_dist < 0.0005 && dist_rot < 20 * M_PI / 180)) {
		ROS_WARN("move commands that cover a distance between 0 and 5 mm or sole rotations less than 20° are not supported!");
		resp.success = false;
		return true;
	}
	...
	if (move_lin_resp.success) {
		...
		ROS_WARN("aborting, a move command with an invalid goal position was sent");
			       ...
			       rll_planning_project::CheckPath::Response &resp)
{
	...
	if ((move_dist < 0.005 && move_dist != 0) || ( move_dist == 0 && dist_rot < 20 * M_PI / 180)) {
		ROS_WARN("check path requests that cover a distance between 0 and 5 mm or sole rotations less than 20° are not supported!");
